...
class Trainer:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        return loss.detach() / self.args.gradient_accumulation_steps, forward_time, backward_time
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if hasattr(model, "model"):
                if hasattr(model.model, 'save_indicators'):
                    if model.model.save_indicators  and description=="Prediction":
                        with open('indicators.jsonl', 'a+') as file:
                            indicies = torch.nonzero(model.model.indicators.squeeze(0), as_tuple=False)[:, 1]
                            json.dump({'indicies': indicies.tolist(), "input_ids": inputs["input_ids"].tolist(), "labels": labels.tolist()}, file)
                            file.write('\n')  
                
                if hasattr(model.model.encoder, 'save_scores'):
                    if model.model.encoder.save_scores and description=="Prediction":
                        with open('scores.jsonl', 'a+') as file:
                            json.dump({'scorer_scores': model.model.encoder.scorer_scores, "average_attn_scores": model.model.encoder.average_attn_scores}, file)
                            file.write('\n')  
